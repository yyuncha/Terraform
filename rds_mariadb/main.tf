...
resource "aws_ssm_parameter" "mariadb" {
    ...
    description = "The parameter of MySQL DB"
    type        = "SecureString"
    value       = random_password.password.result

    tags = merge(var.tags, { Name = var.parameter_store_name })
}
...
resource "aws_security_group" "mariadb" {
  ...
  ingress {
    description      = "DB Service from Application"
    from_port        = var.port
    to_port          = var.port
    protocol         = "tcp"
...
resource "aws_iam_role" "mariadb" {
  ...
  tags = merge(var.tags, { Name = var.monitoring_role_name })
}
...
resource "aws_db_option_group" "mariadb" {
    ...
    tags = merge(var.tags, { Name = var.option_group_name })
